...
Rgb **resize(Rgb **arr, int H, int W, int k)
{
    int new_h = H * k;
    int new_w = W * k;
    int rs = row_size(new_w);
...
static void parse_cli(int argc, char *argv[], AppState *st)
{
    ...
    while ((opt = getopt_long(argc, argv, "lvm:s:e:c:t:n:r:u:d:io:h", long_opts, &idx)) != -1)
    {
        ...
        case 'R':
            ...
            break;
        case 'S':
            if (sscanf(optarg, "%d", &st->k) != 1)
                exit_err(ERR_ARGS, "Ошибка: неверный формат --K");
            break;
        case 'i':
            st->do_info = 1;
            st->act_count++;
...
int main(int argc, char *argv[])
{
    ...
    {
        Rgb **new_arr = resize(arr, H, W, st.k);
        free_arr(arr, H);
        arr = new_arr;
        H = H * st.k;
        W = W * st.k;
